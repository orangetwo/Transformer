# Author   : Orange
# Coding   : Utf-8
# @Time    : 2021/9/8 12:35 下午
# @File    : transformer_xlm.py
...
def get_masks(slen, lengths, causal):
	...
	# encoder mask : (batch size,  seq sequence)
	mask = alen < lengths[:, None]

	# attention mask is the same as mask, or triangular inferior attention (causal)
	if causal:
		# triangular mask: 上三角mask
		# (batch size, seq sequence, seq sequence)
...
class TransformerModel(nn.Module):
	...
	def generate_beam(self, src_enc, src_len, tgt_lang_id, beam_size, length_penalty, early_stopping, max_len=200):
		...
		# src_enc.unsqueeze(1).expand((bs, beam_size) + src_enc.shape[1:]) :  src_enc : (batch size, seq length) - >
		#                                                                        (batch size, beam size, seq length)

		# src_enc : (batch size, seq length) -> (batch size, beam size, seq length)
		src_enc = src_enc.unsqueeze(1).expand((bs, beam_size) + src_enc.shape[1:]).contiguous().view(
			(bs * beam_size,) + src_enc.shape[1:])

		# src_len : (batch size, ) -> (batch size * beam size)x
		...
		# positions : (max length, batch size * beam size)
		positions = src_len.new(max_len).long()
